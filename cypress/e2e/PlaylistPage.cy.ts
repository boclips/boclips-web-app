...
context('Playlist page', () => {
  ...
  function movePiece(id: string, x, y) {
    cy.get(`[data-qa="data-${id}"]`);
  }
  ...
  it('can rearrange playlist (drag n drop)', () => {
    ...
    cy.findAllByText('video title').then((it) => {
      ...
      expect(item1.getAttribute('data-qa')).equals('data-video-1');
      expect(item2.getAttribute('data-qa')).equals('data-video-2');
      expect(item3.getAttribute('data-qa')).equals('data-video-3');
    });
    ...
    cy.findAllByText('video title').then((it) => {
      ...
      expect(item1.getAttribute('data-qa')).equals('data-video-2');
      expect(item2.getAttribute('data-qa')).equals('data-video-1');
      expect(item3.getAttribute('data-qa')).equals('data-video-3');
    });
